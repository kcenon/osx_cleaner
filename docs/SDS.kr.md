...
이 문서는 OSX Cleaner 소프트웨어의 상세 설계 명세를 정의합니다. SRS(Software Requirements Specification)에서 정의된 기술 요구사항을 구현 가능한 설계 명세로 변환합니다.
...
**설계 제외 항목**:
- GUI 상세 설계 (Phase 3 이후)
- Enterprise 기능 (Phase 4)
- 테스트 케이스 상세 (별도 문서)
...
| 원칙 | 설명 | 적용 |
|-----|------|------|
| **Safety First** | 시스템 안정성 최우선 | 모든 삭제 작업 전 안전성 검증 |
| **Modularity** | 독립적 모듈 구조 | 정리 대상별 독립 모듈 |
| **Fail-Safe** | 실패 시 안전한 상태 | 오류 발생 시 작업 중단, 롤백 |
| **Extensibility** | 확장 가능한 구조 | 설정 기반 정리 대상 추가 |
| **Transparency** | 투명한 작업 수행 | 모든 작업 로깅 및 사용자 확인 |
...
| 08-automation-scripts.md | 0.1.0.0 | 자동화 스크립트 참조 |

### 1.5 Technology Stack

#### SDS-TECH-001: Swift + Rust Hybrid Architecture

OSX Cleaner는 **Swift + Rust 하이브리드** 기술 스택을 채택합니다.
...
| 레이어 | 언어 | 책임 | 선택 이유 |
|--------|------|------|-----------|
| **Presentation** | Swift | CLI (ArgumentParser), GUI (SwiftUI) | macOS 네이티브 API, SwiftUI 통합 |
| **Service** | Swift | 비즈니스 로직 조율 | 타입 안전성, async/await |
| **Core - Config/Logger** | Swift | 설정 관리, 로깅 | Foundation 프레임워크 활용 |
| **Core - Engine** | Rust | 파일 스캔, 정리 실행 | 메모리 안전성, 최고 성능 |
| **Core - Safety** | Rust | 안전 등급 계산, 검증 | 정확성, 부작용 없는 함수형 처리 |
| **Infrastructure** | Rust | 파일시스템 추상화 | 병렬 처리, 메모리 효율성 |
| **Scripts** | Shell | launchd 통합, 설치 | macOS 네이티브, 단순성 |

#### 기술 스택 상세

| 범주 | 기술 | 버전 | 용도 |
|------|------|------|------|
| **Swift** | Swift | 5.9+ | 주 개발 언어 |
| | Swift Package Manager | - | 의존성 관리 |
| | ArgumentParser | 1.3+ | CLI 파싱 |
| | SwiftUI | - | GUI (Phase 3) |
| **Rust** | Rust | 1.75+ | 성능 크리티컬 모듈 |
| | Cargo | - | 빌드/의존성 관리 |
| | rayon | 1.8+ | 병렬 처리 |
| | walkdir | 2.4+ | 디렉토리 순회 |
| | serde | 1.0+ | 직렬화/역직렬화 |
| **FFI** | cbindgen | 0.26+ | Rust → C 헤더 생성 |
| | swift-bridge | - | Swift-Rust 바인딩 (선택) |
| **Build** | Make / Just | - | 통합 빌드 스크립트 |
| | Xcode | 15+ | Swift 빌드 |
| **Test** | XCTest | - | Swift 단위 테스트 |
...
│   │   ├── Config/
│   │   └── Logger/
│   └── OSXCleanerGUI/          # SwiftUI 앱 (Phase 3)
│
├── rust-core/                  # Rust 소스
│   ├── Cargo.toml
│   ├── src/
...
│
└── Tests/                      # 테스트
    ├── OSXCleanerKitTests/     # Swift 테스트
    └── rust-core/tests/        # Rust 테스트
```
...
|-------|----------|---------------|------------|
| **Presentation** | Swift/Shell | 사용자 상호작용 | CLI, GUI, launchd Agent |
| **Service** | Swift | 비즈니스 로직 조율 | Analyzer, Cleaner, Scheduler |
| **Core** | Swift + Rust | 핵심 기능 구현 | Config, Logger (Swift) / Safety, Scanner, Cleaner, Target (Rust) |
| **Infrastructure** | Rust | 시스템 추상화, 성능 최적화 | FileSystem, Process, Parallel Worker |
| **FFI** | C-ABI | Swift-Rust 연결 | libosxcore, osxcore.h |
...
**Purpose**: 모든 정리 대상에 대한 안전성 검증 수행
...
**Purpose**: 디스크 사용량 분석 및 정리 가능 공간 추정
...
check_disk_threshold() {
    ...
    if [[ "$usage" -ge "$critical" ]]; then
        send_notification "critical" "디스크 공간 위험" \
            "디스크 사용률이 ${usage}%입니다. 즉시 정리가 필요합니다."
        return 2
    elif [[ "$usage" -ge "$threshold" ]]; then
        send_notification "warning" "디스크 공간 경고" \
            "디스크 사용률이 ${usage}%입니다."
        return 1
    fi
...
⚠️ 주의사항
   - 일부 앱의 첫 실행이 느려질 수 있습니다
   - Time Machine 마지막 백업: 2시간 전
...
*이 문서는 SRS v0.1.0.0을 기반으로 작성되었으며, SRS 변경 시 함께 업데이트되어야 합니다.*
